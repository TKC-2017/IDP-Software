...
void left(int spiral) {
  if (spiral % 4 == 1) {
    // facing east
    intended_heading = north;
    while (heading() - intended_heading != 0){
...
void right_wo_compass() {
    Serial.println("Function: right_wo_compass");
    myMotor_left->setSpeed(100);
    myMotor_left->run(FORWARD);
    myMotor_right->setSpeed(100);
    myMotor_right->run(BACKWARD);
...
void left_wo_compass() {
    Serial.println("Function: left_wo_compass");
    myMotor_left->setSpeed(100);
    myMotor_left->run(BACKWARD);
    myMotor_right->setSpeed(100);
    myMotor_right->run(FORWARD);
...
int LDR1() {
    ...
    if (LDR1 < 190) {
        ...
        // return colour based on min reading
        if (min_reading < 190 && min_reading > 100) {
            return 2;
        }
        if (min_reading < 100) {
            return 1;
...
int LDR3() {
    ...
    if (LDR3 < 250) {
        ...
        // return colour based on min reading
        if (min_reading < 250 && min_reading > 100) {
            return 2; //red
        }
        if (min_reading < 100) {
            return 1; //yellow
...
int detect() {
    ...
    Serial.println(L5);
        ...
        if (max(max(max(a, b), max(c, d)), max(e, f)) == 1) {
         digitalWrite(2, HIGH);
         Serial.println("Detection: yellow detected")
         return 1;
         }
        if (max(max(max(a, b), max(c, d)), max(e, f)) == 2) {
         digitalWrite(3, HIGH);
         Serial.println("Detection: red detected")
         return 2;
         }
